...
public abstract class BaseReportManager implements ReportManager {
	...
	public enum Category {
		OVERVIEW, DAILY, DATA_EXPORT, DATA_QUALITY, MONITORING
	};
	...
	public String getMessageCodePrefix() {
		return "isanteplusreports." + (getName() != null ? getName() : "") + ".";
	}
	...
	public Parameter getStartDateParameter() {
		return new Parameter("startDate", "isanteplusreports.parameter.startDate", Date.class);
	}
	
	public Parameter getEndDateParameter() {
		return new Parameter("endDate", "isanteplusreports.parameter.endDate", Date.class);
	}
	
	public Parameter getLocationParameter() {
		return new Parameter("location", "isanteplusreports.parameter.location", Location.class);
	}
	
	public Parameter getTotalParameter() {
		return new Parameter("total", "isanteplusreports.parameter.total", Integer.class);
	}
	...
	protected ReportDesign xlsReportDesign(ReportDefinition reportDefinition, String templateName, String repeatingSections)
	        throws IOException {
		
		InputStream is = OpenmrsClassLoader.getInstance().getResourceAsStream(
		    "org/openmrs/module/isanteplusreports/reportTemplates/" + templateName + ".xls");
		byte[] excelTemplate = IOUtils.toByteArray(is);
		
		ReportDesign design = new ReportDesign();
		design.setName("isanteplusreports.output.excel");
	...
	protected ReportDesign csvReportDesign(ReportDefinition reportDefinition) {
		ReportDesign design = new ReportDesign();
		design.setName("isanteplusreports.output.csv");
		design.setReportDefinition(reportDefinition);
	...
	protected ReportProcessorConfiguration constructSaveToDiskReportProcessorConfiguration() {
		...
		ReportProcessorConfiguration saveToDiskProcessorConfiguration = new ReportProcessorConfiguration("saveToDisk",
		        DiskReportProcessor.class, saveToDiskProperties, true, false);
