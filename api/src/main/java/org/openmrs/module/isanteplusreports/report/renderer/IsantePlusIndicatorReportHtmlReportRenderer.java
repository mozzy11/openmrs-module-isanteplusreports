...
public class IsantePlusIndicatorReportHtmlReportRenderer extends ReportDesignRenderer {
	...
	public void render(ReportData reportData, String argument, OutputStream out)
		...
		for (String key : reportData.getDataSets().keySet()) {
			DataSet dataset = reportData.getDataSets().get(key);
					...
					if (param != null) {
						...
						try {
							if (param.getName().equals("startDate") || param.getName().equals("endDate")) {
								w.write("<td>"
										+ formater.format(parseFormater.parse(
												dataset.getContext().getParameterValue(param.getName()).toString()))
										+ "</td>");
							} else {
			 ...
			 * w.write("<th>"+column.getName()+"</th>"); } w.write("</tr>");
			 */
