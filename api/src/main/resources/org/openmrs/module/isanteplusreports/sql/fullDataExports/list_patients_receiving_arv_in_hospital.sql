...
FROM isanteplus.patient pat, isanteplus.patient_dispensing pdisp,
    (select pdi.patient_id,max(pdi.dispensation_date) as dispensation_date from isanteplus.patient_dispensing pdi WHERE pdi.voided <> 1 AND pdi.dispensation_date BETWEEN :startDate AND :endDate group by 1)B
