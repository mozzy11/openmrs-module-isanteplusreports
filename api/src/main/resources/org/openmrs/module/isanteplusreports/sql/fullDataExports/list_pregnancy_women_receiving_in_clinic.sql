...
(select pap.patient_id, MAX(pap.start_date) as start_date FROM isanteplus.patient_pregnancy pap 
  WHERE (pap.start_date between :startDate AND :endDate) OR (pap.end_date between :startDate AND :endDate) GROUP BY 1) B
